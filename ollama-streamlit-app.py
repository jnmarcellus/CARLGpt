...
def main():
    st.markdown("<h1 style='text-align: center;'>CARLGpt (Read Only Mode)</h1>", unsafe_allow_html=True)
    st.markdown("<h2 style='text-align: center;'>Corporate Assistant for Rapid Lookups</h2>", unsafe_allow_html=True)
    #st.title("CARLGPT (Read Only Mode)")  # Set the title of the Streamlit app
    logging.info("App started")  # Log that the app has started

    # Sidebar for model selection
    model = st.sidebar.selectbox("Choose a model", ["llama3.1", "tinyllama", "llama3"])
    #model = st.sidebar.selectbox("Choose a model", ["tinyllama", "llama3", "phi3", "mistral"])
